...
def demo():
    for i in range(3):
    #藏头诗
        print(model.predict_hide('唐诗宋词'))
    #给出第一句话进行预测
    print(model.predict_sen('明月几时有，'))
    #给出第一个字进行预测
    print(model.predict_first('君'))
    #随机抽取第一句话进行预测
    print(model.predict_random(temperature=1))

def main():
    while 1:
        text = input("text:")
        sentence = model.predict_first(text)
        print(sentence)
